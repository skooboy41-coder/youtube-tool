...
.block-container {padding:20px 40px;}
.divider {border-left:1px solid #222;height:100%;}
.card {
    display:grid;
...
.thumb {width:120px;border-radius:10px;}
...
def parse_duration(d):
    try:
        h = m = s = 0
        if "H" in d:
            h = int(d.split("H")[0].replace("PT",""))
        if "M" in d:
            m = int(d.split("M")[0].split("H")[-1])
        if "S" in d:
            s = int(d.split("S")[0].split("M")[-1])
    ...
    if st.button("분석 시작"):
        ...
        for r in results:
            ...
            <div class="card">
                ...
                <div><a class="btn" href="{url}" target="_blank">영상 보기</a></div>
            </div>
            """, unsafe_allow_html=True)
...
with col2:
    ...
    if st.button("분석"):
        st.write("👉 제목: 감정 + 결과 강조 부족")
        st.write("👉 썸네일: 대비 부족 / 클릭 유도 약함")
